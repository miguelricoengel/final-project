...
function Buble({ text, size }) {
    ...
    return (
      <div className="relative z-0 h-full w-full">
        <div className="absolute inset-0 z-0 flex items-center justify-center">
          <div className="color h-60 w-60 animate-blob rounded-full bg-blue-300 opacity-40 mix-blend-multiply blur-xl filter"></div>
  ...
  return (
    <div
      ...
      className={`draggable-div ball2 flex cursor-grab select-none items-center justify-center transition-shadow duration-300 hover:drop-shadow-lg ${getSizeClassName()}`}
      style={{
    ...
    >
      ...
      <div className="relative z-0 h-full w-full">
        {["medium", "large"].includes(size) && background()}
      </div>
    </div>
  );
}
...
Buble.propTypes = {
  ...
  size: PropTypes.oneOf(["xs", "small", "medium", "large"]).isRequired,
